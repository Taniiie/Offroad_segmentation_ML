...
# ---------------- CONFIG ----------------
MODEL_PATH = "checkpoints/best_model_optimized.pth"
IMG_SIZE = 512
NUM_CLASSES = 10
...
def process_image(image, model, threshold=0.5):
    ...
    return img_resized, color_mask, overlay, conf.mean(), mask
...
if uploaded_file is not None:
    ...
    with st.spinner("Running segmentation..."):
        img_resized, color_mask, overlay, mean_conf, mask = process_image(image, model, threshold)
    ...
    with c2:
        if show_raw_mask:
            ...
            st.image(color_mask, width='stretch')
        else:
        ...
        if samples:
            ...
            for i, s in enumerate(samples):
                with cols[i]:
                    ...
                    st.image(img, caption=s, width='stretch')
